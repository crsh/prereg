...
| Manipulated variables include manipulation checks or explain why not |    ___ or NA |
...
| Angulation: If acquisition not aligned with scanner axes, specified    |     ___ or NA |
...
<!-- For each piece of software used, give the version number (or, if no version number is available, date of last application of updates). If any subjects required different processing operations or settings in the analysis, those differences should be specified explicitly -->
...
**General issues**
<!-- For novel methods that are not described in detail in a separate paper, provide explicit description and validation of method. Remember to include package and package version used for each test. -->
