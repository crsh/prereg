---
title           : "My multiverse analysis preregistration (Flournoy et al., 2022)"
...
3. What construct is the dependent variable, effect, or outcome?

This is a crucial step, as it will strongly determine the criterion type you choose to compare across specifications (see Section 5). You may also need to clarify this after considering some certain decision points and discovering that they represent comparisons of interest rather than specifications against which we test robustness (see Section 3.1). -->
...
# Decisions
## Covariates
<!-- What are other constructs that are involved in, or somehow affect, the associations of interest? These could be mediators, moderators, confounds, or constructs that impact the effects, even if they don’t impact the causes. These other constructs may be from competing causal models---we’ll tackle this below. -->
...
## Other decisions
<!-- It’s possible the above set of headings is not exhaustive. Include other decisions as well. -->
...
3. Consider amending Section 1 in light of this new information. -->
...
3. Type E: the different possible combinations of options for these decisions will create the set of analyses for each multiverse.

Using the above organizational strategy, describe, your multiverse. To get a sense of the size of the set of the analyses in the multiverse, multiply the number of options for each decision together. If there are two Type E decisions each with three options, and one Type U decision, with two options, we would have two separate multiverses, each with ___ × ___ = ___ observations of the criterion.
...
4. How will the results of this multiverse analysis inform future research on this domain? -->
